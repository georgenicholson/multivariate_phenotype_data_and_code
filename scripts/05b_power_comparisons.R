...
###############################################
#UV/MV global scatterplot (Figure S3)
...
if (control$output_to_dropbox) {
  ...
  file.rename(from = paste(control$figure_dir, "/", fnamc, sep = ""), 
              to = file.path(control$figure_dir, "Figure_S3.jpg"))
}
...
for (numc in save.prop) {
  write.table(formatC(100 * eval(as.name(numc)), digits = 1, format = "f"), 
              file = paste(control$dropbox_text_numbers_dir, "/", numc, ".txt", sep = ""),
              col.names = F, 
              row.names = F, 
              quote = F)
...
if (control$output_to_dropbox) {
  ...
  file.rename(from = paste(control$figure_dir, "/", fnamc, sep = ""), 
              to = file.path(control$figure_dir, "Figure_5.jpg"))
}
...
###############################################
#Plot estimated power by phenotype (Figure S4)
##########################################
...
if (control$output_to_dropbox) {
  ...
  file.rename(from = paste(control$figure_dir, "/", fnamc, sep = ""), 
              to = file.path(control$figure_dir, "Figure_S4.jpg"))
}
